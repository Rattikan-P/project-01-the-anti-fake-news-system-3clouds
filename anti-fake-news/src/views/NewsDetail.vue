    ...
    <div class="text-center py-20">
      <div class="inline-flex flex-col items-center w-full max-w-md">
        ...
        <span class="text-gray-500 text-xl"
          >Loading news details... {{ Math.round(loadingProgress) }}%</span
        >
      </div>
    </div>
  </div>
    ...
    <div class="bg-white shadow-lg rounded-xl overflow-hidden">
      <div class="p-6">
        ...
        <div class="flex items-center mb-6 space-x-4 text-sm text-gray-600">
          ...
          >
            {{
              news.status === 'fake' ? 'Fake News' : news.status === 'not fake' ? 'Real' : 'Equal'
            }}
          </span>
          <p>
        ...
        <div class="mb-6">
          <div class="flex space-x-8 border-b border-gray-200">
            ...
            >
              All Comments
              <div
                v-if="isTabSwitching && pendingTab === 'comments'"
...
const loadNewsDetail = async () => {
  try {
    ...
    await Promise.all([newsStore.fetchNews(), new Promise((resolve) => setTimeout(resolve, 800))])
